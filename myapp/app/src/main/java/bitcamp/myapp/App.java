...
public class App {
    ...
    String[][] subMenus = {
        ...
        {"등록", "목록", "조회", "변경", "삭제"},
        {"등록", "목록", "조회", "변경", "삭제"},
        {}// 도움말의 관련된 서브 메뉴들이 없더라도 공간을 잡아줘야 인덱스 오류가 안 남
    };
    ...
    void printMenu() {
        ...
        for (int i = 0; i < this.mainMenus.length; i++) {
            if (this.mainMenus[i].equals("종료")) {
                System.out.printf("%s%d. %s%s\n", (boldAnsi + redAnsi), (i + 1), this.mainMenus[i],
                    resetAnsi);
            } else {
        ...
        while (true) {
            try {
                ...
                if (command.equals("menu")) {
                    ...
                    String menuTitle = this.getMenuTitle(menuNo, this.mainMenus); // 설명하는 변수
                    if (menuTitle == null) {
                        System.out.println("유효한 메뉴 번호가 아닙니다.");
                    } else if (menuTitle.equals("종료")) {
                        break;
                    } else {
                        this.processMenu(menuTitle, this.subMenus[menuNo - 1]);
