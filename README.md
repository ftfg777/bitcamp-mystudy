...
윈도우와 유/리눅스의 엔터 구분
윈도우에서는 0D 0A가 한쌍으로 와야 줄바꿈으로 인식
유/리눅스는 0A

##### 문자집합 - 조합형(2byte) <= 아래아한글
초성(5bit), 중성(5bit), 종성(5bit)

* 각 5비트씩 할당 후 해당 문자에 맞는 코드값을 2진수로 변환  
* 15비트 앞에 최상위 비트 1(한글 표시)을 더하여 2바이트(byte)를 완성
* 16진수 변환 시 맨앞부터 4비트씩 자르기
* 값이 없을 시 채움
...
- 박 
  - 2진수 변환 => ㅂ(01001) ㅏ(00011) ㄱ(00010)
  - 최상위 비트1 더하기 => 1010 0100 0110 0010
  - 16진수 변환 => 0xA462
